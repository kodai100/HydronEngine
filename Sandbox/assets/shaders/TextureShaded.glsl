...
void main()
{
	...
	vec3 eyeDir = -normalize(v_Position - u_CameraPos);

	vec3  invLight  = normalize(u_InvTransform * vec4(-u_LightDirection, 0.0)).xyz;
    vec3  invEye    = normalize(u_InvTransform * vec4(eyeDir, 0.0)).xyz;
